            ...
            <div class="mx-auto  max-w-7xl">
                ...
                @foreach ($cartItems as $cartItem)
                    ...
                    <div class="flex flex-col md:flex-row justify-center items-center py-5 border-b border-gray-200">
                        ...
                        <input type="hidden" name="books[{{ $loop->index }}][image]"
                            value="{{ asset('images/' . $cartItem->book->image_path) }}">
                        <input type="hidden" name="books[{{ $loop->index }}][name]"
                            value="{{ $cartItem->book->bookName }}">
                        <input type="hidden" name="books[{{ $loop->index }}][price]"
                            value="{{ $cartItem->book->price }}">
                        <input type="hidden" name="books[{{ $loop->index }}][quantity]"
